...
public class Eventos01MouseEvent extends Application {
  ...
  public void start(Stage primaryStage) {
    ...
    EventHandler<MouseEvent> handlerPropagacion = new EventHandler<MouseEvent>() {
      @Override
      public void handle(MouseEvent e) {
        manejador(e, "handlerPropagacion (usado con addEventHandler)");
      }
    };

    // El manejador anterior podríamos haberlo implementado usando expresiones lambda.

    EventHandler<MouseEvent> handlerCaptura = e -> manejador(e, "handlerCaptura (usado con addEventFilter)");
     ...
     * 
     * 2. Fase de captura: el nodo raíz (de la ruta) distribuye el evento, dicho evento recorre la jerarquía
     *    desde la parte superior, si alguno de los nodos ha registrado un manejador de eventos con filtro
     *    (addEventFilter) se invoca, esto ocurre hasta llegar a la parte inferior del recorrido que termina
     *    con el objeto que originó el evento.
     *    
    ...
    button1.addEventHandler(MouseEvent.MOUSE_CLICKED, handlerPropagacion);
    button2.addEventHandler(MouseEvent.MOUSE_CLICKED, handlerPropagacion);
    button2.addEventFilter(MouseEvent.MOUSE_CLICKED, handlerCaptura); // MAL USADO, no hacedlo así
    ...
    textField.addEventFilter(MouseEvent.MOUSE_CLICKED, handlerCaptura);
    
    visor.addEventHandler(MouseEvent.MOUSE_CLICKED, handlerPropagacion);
    visor.addEventFilter(MouseEvent.MOUSE_CLICKED, handlerCaptura);
    
    scene.addEventHandler(MouseEvent.MOUSE_CLICKED, handlerPropagacion);
    scene.addEventFilter(MouseEvent.MOUSE_CLICKED, handlerCaptura);

    primaryStage.addEventHandler(MouseEvent.MOUSE_CLICKED, handlerPropagacion);
    primaryStage.addEventFilter(MouseEvent.MOUSE_CLICKED, handlerCaptura);
   ...
   */
  protected void manejador(Event e, String registro) {
    numOcurrencias++;
    visor.appendText("Soy el manejador de eventos " + registro + "\n");
    ...
    visor.appendText("Ocurrencia número: " + numOcurrencias + "\n");
    visor.appendText("------------------------------------------\n");
  }
